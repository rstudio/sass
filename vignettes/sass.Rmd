...
This allows downstream `sass_layer()`s to be `sass_layers()`d into a single layer, where those downstream layers are granted higher priority. More specifically, this means:
...
sass(sass_layers(layer1, layer2))
```
