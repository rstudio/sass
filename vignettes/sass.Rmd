...
The following application will demonstrate this example. We'll use Dean Attali's [colourpicker](https://github.com/daattali/colourpicker) to choose a color and write it to `_variables.scss`. 

Then the app compiles `new-style.scss` and renders the new style.

```{r echo = FALSE}
knitr::include_app("http://127.0.0.1:3685", height = "600px")
```

## Functions

If we choose a background color that is too dark, we won't be able to read the title **New Application**. Therefore, we need to make the font color depend on the background color.
...
Now the header font color [depends](https://stackoverflow.com/a/3943023/6637133) on the background color, switching between black and white as necessary:

```{r echo = FALSE}
knitr::include_app("http://127.0.0.1:3415", height = "400px")
```
...
Here's we'll change the width of the image based on a slider:
...
```{r echo = FALSE}
knitr::include_app("http://127.0.0.1:7561", height = "600px")
```
...
```{r}
library(sass)
compile_sass(text = new_style, options = opts(output_style = "compressed"))
```
